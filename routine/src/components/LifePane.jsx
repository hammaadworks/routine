...
import { Star, CheckCircle2, Plus, Trash2, Pencil, Activity, Clock, X, Rocket, Palette, ChevronDown } from 'lucide-react';
...
export default function LifePane({ 
  ...
  const deleteLifeGoal = (id, text) => {
    setConfirmConfig({
      ...
      message: `Are you sure you want to delete the life goal: "${text}"? Sprint and routine goals linked to it will be unlinked.`,
      isDanger: true,
      onConfirm: () => {
        setLifeGoals(lifeGoals.filter(g => g.id !== id));
        
        if (sprintGoals && setSprintGoals) {
          setSprintGoals(sprintGoals.map(g => g.lifeGoalId === id ? { ...g, lifeGoalId: '' } : g));
        }
  ...
  const getLinkedCount = (goal) => {
    let count = 0;
    if (sprintGoals) {
      count += sprintGoals.filter(g => g.lifeGoalId === goal.id).length;
    }
    if (routineGoals) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto', flex: 1, padding: '8px 12px 8px 4px', marginTop: '-8px' }}>
            ...
            return (
              <div key={goal.id} className={`item-card ${goal.completed ? 'scratched' : ''}`} style={{ cursor: 'default', position: 'relative', height: '48px', padding: '0 12px', display: 'flex', alignItems: 'center', ...bgStyle }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                  ...
                  <div 
                  title="Total Linked Sprint & Routine Goals"
                  style={{
                  position: 'absolute',
                  top: '-8px',
                  right: '-8px',
          ...
          {lifeGoals.length === 0 && (
            <div style={{ 
              ...
              <div style={{ fontSize: '12px', marginTop: '4px', opacity: 0.7 }}>Add long-term aspirations that transcend routine versions.</div>
            </div>
          )}
        </div>
      </div>
