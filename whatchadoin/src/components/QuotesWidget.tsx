...
const DEFAULT_QUOTES: Quote[] = [
  ...
  { id: '4', text: "Stand up and act like the disciplined routine of you." },
  { id: '5', text: "One focused action today builds tomorrow's income." },
  { id: '6', text: "Discipline beats motivation, especially on lazy days." },
  { id: '7', text: "Do the small habit now. Let compounding cook." },
  { id: '8', text: "Your identity decides your income, move accordingly." },
  { id: '9', text: "Lock in daily. Success loves predictable effort." },
  { id: '10', text: "Consistency is how underachievers turn unstoppable." },
  { id: '11', text: "Build habits that make success automatic." },
  { id: '12', text: "Focus is currency, stop spending it on nothing." },
  { id: '13', text: "Act broke in effort, rich in discipline." },
  { id: '14', text: "You don't feel disciplined. You act disciplined." },
  { id: '15', text: "Your routine is your wealth strategy." },
  { id: '16', text: "Show up tired. Consistency still counts." },
  { id: '17', text: "Discipline today funds freedom tomorrow." },
  { id: '18', text: "Stack disciplined days. Watch money follow." },
  { id: '19', text: "Habits don't ask how you feel. Neither should you." },
  { id: '20', text: "Get up. Become the person who doesn't quit." }
];
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
        ...
        <button onClick={handleShuffle} className="icon-btn" style={{ padding: '4px', opacity: 0.7 }} title="Shuffle">
          <Shuffle size={12} />
        </button>
        <button onClick={() => setShowModal(true)} className="icon-btn" style={{ padding: '4px', opacity: 0.7 }} title="Manage Quotes">
          <Settings size={12} />
        </button>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <input 
              ...
              style={{ flex: 1, padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--panel-border)', background: 'var(--bg)', color: '#fff' }}
            />
            <button onClick={handleAdd} className="primary" style={{ padding: '8px 12px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Plus size={16} /> Add
            </button>
          </div>
                ...
                {editingId === q.id ? (
                  <div style={{ display: 'flex', gap: '8px', flex: 1 }}>
                    <input 
                      ...
                      style={{ flex: 1, padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--accent)', background: 'var(--bg)', color: '#fff' }}
                      autoFocus
                    />
                    <button onClick={() => saveEdit(q.id)} className="icon-btn" style={{ color: 'var(--accent)' }}><Check size={16} /></button>
                    <button onClick={() => setEditingId(null)} className="icon-btn" style={{ color: 'var(--text-secondary)' }}><X size={16} /></button>
                  </div>
                ) : (
                  <>
                    <div 
                      style={{ flex: 1, fontStyle: 'italic', fontSize: '13px', cursor: 'pointer', color: currentQuoteIndex === idx ? 'var(--accent)' : 'inherit' }}
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      <button onClick={() => startEdit(q)} className="icon-btn" style={{ padding: '4px', color: 'var(--text-secondary)' }} title="Edit"><Edit2 size={14} /></button>
                      <button onClick={() => handleDelete(q.id)} className="icon-btn" style={{ padding: '4px', color: 'var(--danger)' }} title="Delete"><Trash2 size={14} /></button>
                    </div>
                  </>
                )}
              </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '8px' }}>
            <button onClick={() => setShowModal(false)} className="primary" style={{ padding: '8px 24px', color: '#000' }}>Done</button>
          </div>
        </div>
      </BaseModal>
    </>
