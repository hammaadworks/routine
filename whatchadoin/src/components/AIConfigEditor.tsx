...
export default function AIConfigEditor({config, setConfig}: AIConfigEditorProps) {
    ...
    const updateActiveProfile = (updates: Partial<AIProfile>) => {
        const newProfiles = config.profiles.map(p => p.id === activeProfile?.id ? {...p, ...updates} : p);
    ...
    const addProfile = () => {
        ...
        const newProfile: AIProfile = {
            id: newId, name: 'New Profile', provider: 'openai', model: 'gpt-4o', apiKey: '', customEndpoint: ''
        };
        setConfig({
            activeProfileId: newId, profiles: [...config.profiles, newProfile]
        });
    };
    ...
    const deleteProfile = (id: string) => {
        ...
        setConfig({
            activeProfileId: newActiveId, profiles: newProfiles
        });
    };
    ...
    >
        ...
        <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                <div style={{flex: 1}}>
                    ...
                    >
                        {config.profiles.map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}
                    </select>
                </div>
                <div style={{display: 'flex', gap: '4px', marginTop: '24px'}}>
                    <button type="button" onClick={addProfile} className="icon-btn" style={{
                        padding: '10px',
                        background: 'var(--bg)',
                        border: '1px solid var(--panel-border)',
                        borderRadius: '6px'
                    ...
                    <button type="button" onClick={() => deleteProfile(activeProfile?.id || '')} className="icon-btn"
                            style={{
                                padding: '10px',
                                background: 'var(--bg)',
            ...
            <div style={{
                ...
                {activeProfile?.provider === 'custom' && (<div>
                        <label style={{
                            display: 'block',
                            fontSize: '12px',
                            marginBottom: '8px',
