...
- **The 3-Pane Workflow**: A distinct left pane for high-level Routine Goals, a central dynamic Timeline for scheduling, and a right pane for granular Routine/Atomic habits.
...
Getting whatchadoin running locally takes just three commands:
...
1. **Set your period**: In the top left pane, pick a start and end date for your current phase (e.g. your week or two-week routine).
2. **Brainstorm tasks**: Click `+ Add Routine Goal` on the left to add major project milestones, and `+ Add Daily Goal` on the right to add atomic habits (like "Read 10 pages" or "Meditation").
...
### Defining Durations
When creating a Habit, ensure you specify a valid duration constraint. The timeline relies on this to draw the block.
